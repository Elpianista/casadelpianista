        ...
        <div class="summary-dashboard">
            ...
            <div class="summary-view-selectors">
                <button class="btn-view-opt ${moduleIds.length === 6 ? 'active' : ''}" onclick="renderComprehensiveSummary('${studentId}', [1,2,3,4,5,6])">
                    <i class="ri-dashboard-line"></i> <span class="btn-text">Resumen Total</span>
                </button>
                <button class="btn-view-opt ${(moduleIds.length === 1 && moduleIds[0] == stats.studentInfo.nivel) ? 'active' : ''}" onclick="renderComprehensiveSummary('${studentId}', [${stats.studentInfo.nivel}])">
                    <i class="ri-bookmark-line"></i> <span class="btn-text">Módulo Actual (${stats.studentInfo.nivel})</span>
                </button>
                <button class="btn-view-opt" onclick="openCustomModulePicker('${studentId}')">
                    <i class="ri-filter-3-line"></i> <span class="btn-text">Promedio Personalizado</span>
                </button>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    setupProfileTabs();
    setupPriorityPlusMenu();
});

function setupPriorityPlusMenu() {
    const tabsContainer = document.getElementById('profile-tabs');
    const overflowContainer = document.querySelector('.tabs-overflow-container');
    ...
    const updateTabs = () => {
        const containerWidth = tabsContainer.parentElement.offsetWidth - 70; // Reserve space for + button
        const children = Array.from(tabsContainer.querySelectorAll('.profile-tab-btn:not(.in-dropdown)'));

        let accumulatedWidth = 0;
        let overflowed = false;

        // Move all from dropdown back to main container for a fresh calculation
        const inDropdown = Array.from(dropdown.querySelectorAll('.profile-tab-btn'));
        inDropdown.forEach(btn => {
            btn.classList.remove('in-dropdown');
            tabsContainer.appendChild(btn);
        });

        // Re-calculate
        const allButtons = Array.from(tabsContainer.querySelectorAll('.profile-tab-btn'));
        allButtons.forEach(btn => {
            accumulatedWidth += btn.offsetWidth;
            if (accumulatedWidth > containerWidth) {
                btn.classList.add('in-dropdown');
                dropdown.appendChild(btn);
                overflowed = true;
            }
        });

        overflowContainer.style.display = overflowed ? 'block' : 'none';
    };

    // Use ResizeObserver for smooth real-time response
    const observer = new ResizeObserver(() => {
        updateTabs();
    });

    if (tabsContainer.parentElement) {
        observer.observe(tabsContainer.parentElement);
    }

    // Attach to window for external triggering if needed
    window.updateProfileTabOverflow = updateTabs;
...
window.initStudentProfile = function (student) {
    ...
    // Force update of tab overflow
    setTimeout(() => {
        if (window.updateProfileTabOverflow) window.updateProfileTabOverflow();
    }, 50);
};
...
function renderDetailedReportHTML(g, targetId, isAveraged = false) {
    ...
    container.innerHTML = `
        ...
        <div class="report-actions-level-wrapper" style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 0.3rem;">
