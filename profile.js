...
window.initStudentProfile = function (student) {
    ...
    document.getElementById('info-edad').textContent = `${student.edad} años`;
    document.getElementById('info-sexo').textContent = student.sexo;
    document.getElementById('info-cumpleanos').textContent = student.cumpleanos || 'No registrado';

    document.getElementById('info-instrumento').textContent = student.instrumento;
    document.getElementById('info-nivel').textContent = `Nivel ${student.nivel}/6`;
    document.getElementById('info-registro').textContent = student.fechaRegistro;
    document.getElementById('info-frecuencia').textContent = `${student.clasesSemanales || 2}x por semana`;

    document.getElementById('info-apoderado').textContent = student.apoderado;
    document.getElementById('info-celular').textContent = student.celular;
    document.getElementById('info-direccion').textContent = student.direccion;
    document.getElementById('info-referencia').textContent = student.referencia || '-';
...
function renderDetailedReportHTML(g, targetId, isAveraged = false) {
    ...
    const songPills = [
        ...(g.evaluatedSongs || []).map(idx => moduleSongs[idx]?.title).filter(Boolean),
        ...(g.customSongs || [])
    ];
    ...
    container.innerHTML = `
        ...
        <!-- EVALUATED SONGS AREA (Moved to Title position) -->
        ${songPills.length > 0 ? `
        <div class="report-songs-section-mid">
            <h3 class="section-divider-title">MUSICA EN EVALUACION:</h3>
...
window.showGradeEditor = function (index = null) {
    const instrument = getStudentInstrument();
    const capturedId = currentStudentId;
...
function renderModuleSongsSelector(savedEvaluatedIndices) {
    ...
    const masterKey = `master_syllabus_${instrument}_mod_${modId}`;
    const rawData = localStorage.getItem(masterKey);
    const masterData = JSON.parse(rawData || '{"songs":[]}');
    const songs = masterData.songs || [];

    const songSelector = document.getElementById('grade-songs-selector');
    if (songs.length === 0) {
        songSelector.innerHTML = '<div style="padding: 10px; color: #999; font-size: 0.8rem; width: 100%; text-align: center;">No hay canciones registradas en este módulo para este instrumento.</div>';
        updateSelectedSongsCount();
        return;
    }

    songSelector.innerHTML = songs.map((s, idx) => `
        <label class="song-pill-check">
            <input type="checkbox" value="${idx}" ${savedEvaluatedIndices.includes(idx) ? 'checked' : ''} 
...
window.saveGrades = function () {
    ...
    const evaluatedSongs = Array.from(document.querySelectorAll('#grade-songs-selector input:checked')).map(el => parseInt(el.value));
...
window.openSubtaskForm = function (subtaskIdx = null) {
    ...
    document.getElementById('subtask-modal-title').innerHTML = subtaskIdx !== null ? '<i class="ri-edit-line"></i> Editar Subtarea' : '<i class="ri-add-line"></i> Nueva Subtarea';
    ...
    if (songSelect) {
        ...
        masterData.songs.forEach((song, idx) => {
            const option = document.createElement('option');
            option.value = idx;
            option.textContent = song.title; // Assuming song object has a title
            if (subtask && subtask.linkedSongIdx == idx) {
    ...
    // Note: Manual grade field removed from form, grade handled via rubric button

    currentSubtaskImages = subtask ? [...subtask.images] : [];
    renderSubtaskImagesPreview();

    document.getElementById('subtask-form-modal').style.display = 'block';
};
...
window.saveSubtask = function () {
    ...
    // Automatically recalculate main task grade
            ...
            <div style="display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap;">
                ...
                ${st.linkedSongIdx !== undefined && st.linkedSongIdx !== '' ? `
                    <div style="display: flex; align-items: center; gap: 4px; color: #4338ca; font-size: 0.85rem; font-weight: 500; background: #eef2ff; padding: 2px 8px; border-radius: 4px; border: 1px solid #c7d2fe;">
                        <i class="ri-music-2-line"></i> ${masterData.songs[st.linkedSongIdx]?.title || 'Canción'}
                    </div>
                ` : ''}
...
window.openSubtaskView = function (subtaskIdx) {
    ...
    if (viewSongLink) {
        if (subtask.linkedSongIdx !== undefined && subtask.linkedSongIdx !== '') {
            ...
            const masterKey = `master_syllabus_${instrument}_mod_${modId}`;
            const masterData = JSON.parse(localStorage.getItem(masterKey) || '{"songs":[]}');
            const songTitle = masterData.songs[subtask.linkedSongIdx]?.title || 'Canción no encontrada';
